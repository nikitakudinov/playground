...
class NocodedbSupabaseUser extends BaseAuthUser {
  NocodedbSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> nocodedbSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NocodedbSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
